({
	...
	****************************************************/
    ...
    setYearPickList : function(component, event, helper) {
        ...
        //yearList.push(temp);
        for(let i=0;i<=(currentDay.getFullYear()- eoyYear);i++){
            let temp={'label':eoyYear , 'value': eoyYear};
            eoyYear+=1;
            yearList.push(temp);
        }
	...
	****************************************************/
    ...
    updateBreakDown : function(component, event, helper){
        ...
        let commissionId ='';
        let breakDownms = component.get('v.MastermonthlyBreakDowns');
        let monthlyCommissions = component.get('v.MastermonthlyCommissions');
        for(let monthlyCommission in monthlyCommissions){
            if(monthlyCommissions[monthlyCommission].Name.includes(component.get('v.selectedYear')) &&  monthlyCommissions[monthlyCommission].Name.includes(component.get('v.selectedMonth'))){
        ...
        for(let breakDown in breakDownms){
            if(breakDownms[breakDown].Monthly_Commission__c == commissionId){
                let temp = {}
                temp=breakDownms[breakDown];
                filtredMonthlyBreakDownByYear.push(breakDownms[breakDown]);
            }
        }
